...
export async function globRead(globPattern, writer, wait = 0, closeOnEnd = true, binary = false) {
    ...
    const files = await Promise.all(jsfiles.map((x) => {
...
export async function readFolder(folder, writer, maxMemory = 3, pause = 5000) {
...
export function getFileFromFolder(reader, folderPath, writer) {
...
export function unzipFile(reader, writer, outputAsBuffer = false) {
...
export function gunzipFile(reader, writer, outputAsBuffer = false) {
