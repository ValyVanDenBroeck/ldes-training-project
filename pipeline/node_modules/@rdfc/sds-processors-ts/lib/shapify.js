...
import { Extractor } from "./utils/index.js";
export async function shapify(reader, writer, shape) {
    ...
    reader.data(async (x) => {
        ...
        for (const record of records) {
            ...
            const newData = await cbd_extractor.extract(store, record.data.id, shape.id);
            const n_writer = new NWriter({ format: "text/turtle" });
            n_writer.addQuads(quads.filter((x) => x.graph.value ===
                "https://w3id.org/sds#DataDescription"));
            n_writer.addQuads(newData);
            const out_str = await new Promise((res, rej) => n_writer.end((error, result) => {
                if (error)
                    rej(error);
                res(result);
            }));
